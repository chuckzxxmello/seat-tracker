        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 md:py-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <div>
              ...
              <p className="text-slate-600 mt-1 text-xs md:text-sm">Manage attendees and seat assignments</p>
            </div>
        ...
        {activeTab === "delegates" && (
          <div className="space-y-6 md:space-y-8">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
              <Card className="bg-white border-blue-200 p-5 md:p-6 shadow-sm">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-slate-600 text-xs md:text-sm">Checked In</p>
                    <p className="text-2xl md:text-3xl font-bold text-emerald-600 mt-1 md:mt-2">{stats.checkedIn}</p>
                  </div>
              ...
              <Card className="bg-white border-blue-200 p-5 md:p-6 shadow-sm">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-slate-600 text-xs md:text-sm">Pending</p>
                    <p className="text-2xl md:text-3xl font-bold text-amber-600 mt-1 md:mt-2">{stats.pending}</p>
                  </div>
              ...
              <Card className="bg-white border-blue-200 p-5 md:p-6 shadow-sm">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-slate-600 text-xs md:text-sm">Check-in Rate</p>
                    <p className="text-2xl md:text-3xl font-bold text-blue-600 mt-1 md:mt-2">{stats.checkInRate}%</p>
                  </div>
            ...
            <Card className="bg-white border-blue-200 p-5 md:p-8 shadow-sm">
              <h2 className="text-lg md:text-xl font-semibold text-slate-900 mb-4 md:mb-6">Seat Lookup</h2>
            ...
            {searchedSeat && selectedAttendees.length > 0 && (
              <div>
                <Card className="bg-white border-blue-200 p-4 md:p-6 shadow-sm">
                  ...
                  <PathfindingVisualization seatId={searchedSeat} isVip={false} showBackground={false} />
                  <p className="text-xs md:text-sm text-slate-600 mt-4 p-3 md:p-4 bg-blue-50 rounded-lg leading-relaxed">
                    ...
                    <strong>Table {searchedSeat}</strong>. All tables with this seat number are shown above.
                  </p>
                </Card>
              </div>
            )}
            ...
            {selectedAttendees.length > 0 && (
              <div className="space-y-4">
                ...
                {selectedAttendees.map((attendee) => (
                  <Card key={attendee.id} className="bg-white border-blue-200 p-4 md:p-6 shadow-sm">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
                      <div>
                        <h3 className="text-slate-600 text-xs md:text-sm font-medium mb-3 md:mb-4">Attendee</h3>
                        <p className="text-lg md:text-xl font-bold text-slate-900 mb-2">{attendee.name}</p>
                        <p className="text-slate-600 text-xs md:text-sm">{attendee.ticketNumber}</p>
                      </div>
                      <div>
                        <h3 className="text-slate-600 text-xs md:text-sm font-medium mb-3 md:mb-4">Details</h3>
                      ...
                      <div className="flex flex-col justify-between">
                        <div>
                          <h3 className="text-slate-600 text-xs md:text-sm font-medium mb-3 md:mb-4">Actions</h3>
                        </div>
            ...
            {searchSeat && !isSearching && selectedAttendees.length === 0 && (
              <Card className="bg-white border-blue-200 p-6 md:p-8 text-center shadow-sm">
                <p className="text-slate-600 text-sm md:text-base">No attendees found at that seat.</p>
              </Card>
            )}
          </div>
        )}
