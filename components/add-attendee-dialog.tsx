...
export function AddAttendeeDialog({ onClose, onSuccess }: AddAttendeeDialogProps) {
  ...
  const [availableTables, setAvailableTables] = useState<{ id: number; label: string; type: string }[]>([])
    ...
    <Card className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-xl">
        <div className="flex items-center justify-between p-6 border-b border-blue-200 sticky top-0 bg-white">
          <h2 className="text-2xl font-bold text-slate-900">Add New Attendee</h2>
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            <h3 className="text-lg font-semibold text-slate-900 mb-4">Attendee Personal Information</h3>
          ...
          <div className="border-t border-blue-200 pt-6">
            <h3 className="text-lg font-semibold text-slate-900 mb-4">Seating Assignment</h3>
            {isLoadingTables ? (
              <p className="text-slate-500 text-sm">Loading available tables...</p>
            ) : availableTables.length === 0 ? (
              <p className="text-amber-600 text-sm">
                No tables configured. Please set up the venue map in the admin panel first.
              </p>
            ) : (
              <div>
